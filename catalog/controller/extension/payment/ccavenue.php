...
class ControllerExtensionpaymentccavenue extends Controller {
	...
	public function index() {
		...
		$data['action'] = $this->config->get('payment_ccavenue_action');
		...
		$data['access_code'] = $this->config->get('payment_ccavenue_access_code');
		...
		if ($order_info) {
			...
			$merchant_id=$this->config->get('payment_ccavenue_Merchant_Id');  
			...
			$redirect_url=urlencode($this->url->link('extension/payment/ccavenue/callback', '', true));         

			$cancel_url=urlencode($this->url->link('checkout/checkout', '', true));
			...
			$working_key=$this->config->get('payment_ccavenue_workingkey');

			$access_code=$this->config->get('payment_ccavenue_access_code');
			...
			$data['checkout_method'] = $this->config->get('payment_ccavenue_checkout_method');

			if($data['checkout_method']=='iframe')

			{

			$merchant_data .='&integration_type=iframe_normal&language=EN'; 
	...
	public function callback() {
		...
		$workingKey=$this->config->get('payment_ccavenue_workingkey');
		...
		if ($order_info) {
					...
					$order_status_id = $this->config->get('payment_ccavenue_completed_status_id');
					...
					$order_status_id = $this->config->get('payment_ccavenue_failed_status_id');
					...
					$order_status_id = $this->config->get('payment_ccavenue_pending_status_id');
