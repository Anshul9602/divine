<?php
class ControllerCommonCelebrities extends Controller
{
...
// celebrities img
		...
		$style_categories = $this->model_catalog_category->getCategories(93);
		...
		foreach ($style_categories as $category) {
         $product_total = $this->model_catalog_product->getTotalProducts($category['category_id']);
			...
			$data['style_categories'][] = array(
				...
				'href'     => $this->url->link('product/category', 'path=93_' . $category['category_id'])
			);
		}
...
// echo "</pre>";
		...
		$this->response->setOutput($this->load->view('common/celebritie', $data));
	}
}
