...
if($category_info['name'] == 'DIAMOND JEWELLERY'){
	$href = '#';}
else if($category_info['name'] == 'POLKI JEWELLERY'){
	$href = '#';
}else{
...
$data['total_products1'] = $product_info['quantity'];
			...
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize1($result['image'], '500', '500');
				} else {
...
if(file_exists('catalog/model/extension/fbcapidyad.php') && isset($this->request->get['product_id'])) { 
			...
			$data['metal'] = $product_info['sku'];
			$data['stone'] = $product_info['upc'];
			$weight = $product_info['weight'];
		
			$data['stone_cut'] = $product_info['ean'];
			$Weight = (float)$weight;
			if($Weight == 0){
