<?php
class ModelExporterOrderpro extends Model {
	...
	public function getOrders($data = []) {
		// Find Orders
		$sql = "SELECT o.*, op.order_product_id, op.product_id, op.name as product_name, op.model as product_model, op.quantity as product_quantity, op.price as product_price, op.total as product_total_price, op.tax as product_tax, op.reward as product_reward, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM `" . DB_PREFIX . "order_status` os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS order_status FROM `" . DB_PREFIX . "order_product` op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) ";

		if (isset($data['find_order_status'])) {
			$implode = [];

			$order_statuses = explode(',', $data['find_order_status']);
		...
		if (!empty($data['find_order_id'])) {
			$comma_ids = explode(',', $data['find_order_id']);
			...
			if($comma_ids) {
				...
				foreach($comma_ids as $comma_id) {
					if($comma_id) {
						$explode_comma_ids = explode('-', $comma_id);
