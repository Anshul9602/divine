<?php
class ControllerExtensionModuleKeysubmit  extends Controller {
	private $error = array();
	public function loadfrom() {
		
		$user_token= $this->session->data['user_token'];
		$regkey= $this->config->get('moduledata_tmdbundle_key');
		 $url = 'https://www.opencartextensions.in/index.php?route=api/keysubmit&foldername=extension/module&regkey='.$regkey.'&user_token='.$user_token;
	...
	public function index() {
		...
		$this->load->language('extension/module/keysubmit');
		...
		curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query($tmd_extensiondata, '', '&'));
		 ...
		 $status=json_decode($response,true);
			if($status['status']) {
				$keydata=array();
				$keydata['moduledata_tmdbundle_key']=$this->request->post['moduledata_key'];
